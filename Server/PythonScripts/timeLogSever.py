'''
----------------------------------------------------------
 This is server side python script for saving received
 "position" for indoor measurements from mobile device.
 Data has been sent via MQTT.
----------------------------------------------------------
'''
import signal
import sys
...
_topicGPSLog = ["lora/testTimeLog"] # MQTTs topic name
...
_client = mqtt.Client(client_id="timeLogServer") # MQTT client
...
def on_message(client, userdata, msg):
	...
	try:
		...
		pointNumber = None # Orientation point number
		if "pointNumber" in messJson:
			pointNumber = messJson["pointNumber"]
		time = None # Time when orientation point was reached
		if "time" in messJson:
			time = messJson["time"]
		additionalData = None
		if "additionalData" in messJson:
			additionalData = messJson["additionalData"]

		# Insert time and point number from mobile device
		if pointNumber != None and time != None and additionalData != None:
			values = (pointNumber, time, additionalData)
			querry = """INSERT INTO `point_time` (`id`, `point_number`, `timestamp`, `description`) VALUES (NULL, %s, %s, %s);"""
		
			mId = saveToDb(querry, values)

			if mId == -1:
				printAndLog("ERROR SAVING point time info to db.")

	except Exception as e:
		print("\n!!!!!!!!!!!!Exception!!!!!!!!!!!!!")
		printAndLog("EXCEPTION occured while parsin JSON from " + msg.topic + ". Exception: " + str(e))
		printAndLog ("Topic: "+ msg.topic+"\nMessage: " + msgPayloadStr)
		
		if not _exceptionSent[0]:
			# In case of exception send mail to warn user
			msgBody = 'There has been some error while parsing TIME LOG JSON data received form MOBILE DEVICE.\n\n Error description: ' + str(e) + '.\n\nReceived JSON message: ' + msgPayloadStr
			
			sendemail(from_addr    = '',
				to_addr_list = [''],
				cc_addr_list = [], 
				subject      = 'MOBILE DEVICE: Error parsing TIME LOG JSON data', 
				message      = msgBody,
				login        = '',
				password     = '')
...
def printAndLog(msg):
	...
	print(msg)
	try:
		with open(_script_dir[0] + '/timeLog.log', 'a') as resFile:
		...
		if fileData:
			try:
				...
				if "mosquitto" in devSettJson:
					...
					# MQTT settings
					_topicGPSLog[0] = mosqSett["topicTimeLog"]
