...
tFskSettings FskSettings = 
{
	...
	5,             // Power - THIS POWER IS OVERRIDDEN, DOESN'T HAVE THE EFFECT ON OUTPUT POWER FROM TRANSCEIVER

	100000,         // RxBw
    150000,         // RxBwAfc
    true,           // CrcOn
    true,           // AfcOn    
    9             // PayloadLength (set payload size to the maximum for variable mode, else set the exact payload length)
};
...
uint32_t SX1276FskProcess( void )
{
    ...
    case RF_STATE_RX_INIT:
        ...
        Preamble2SyncTimer = RxTimeoutTimer = linux_hal_get_ms();
    ...
    case RF_STATE_RX_SYNC:
        if( ( DIO4 == 1 ) && ( PreambleDetected == false ) )// Preamble
        {
            ...
            Preamble2SyncTimer = linux_hal_get_ms();
        }
        ...
        if( ( SyncWordDetected == false ) && ( PreambleDetected == true ) && ( ( linux_hal_get_ms() - Preamble2SyncTimer ) > Preamble2SyncTimeout ) )
        {
        ...
        if( ( SyncWordDetected == false ) &&
            ( PreambleDetected == false ) &&
            ( PacketDetected == false ) &&
            ( ( linux_hal_get_ms() - RxTimeoutTimer ) > PacketTimeout ) )
        {
    ...
    case RF_STATE_RX_RUNNING:
        ...
        // Packet timeout
        if( ( PacketDetected == false ) && ( ( linux_hal_get_ms() - RxTimeoutTimer ) > PacketTimeout ) )
        {
            RFState = RF_STATE_RX_TIMEOUT;
        }
        break;
    ...
    case RF_STATE_TX_READY_WAIT:
        if( DIO3 == 1 )    // TxReady
        {
            ...
            TxTimeoutTimer = linux_hal_get_ms();
    ...
    case RF_STATE_TX_RUNNING:
        ...
        // Packet timeout
        if( ( linux_hal_get_ms() - TxTimeoutTimer ) > TICK_RATE_MS( 1000 ) )
        {
