...
#define CAD_DONE_BIT_POSITION						2
...
tLoRaSettings LoRaSettings =
{
		...
		20,               // Power - THIS POWER IS OVERRIDDEN, DOESN'T HAVE THE EFFECT ON OUTPUT POWER FROM TRANSCEIVER

		9,                // SignalBw [0: 7.8kHz, 1: 10.4 kHz, 2: 15.6 kHz, 3: 20.8 kHz, 4: 31.2 kHz,
						  // 5: 41.6 kHz, 6: 62.5 kHz, 7: 125 kHz, 8: 250 kHz, 9: 500 kHz, other: Reserved]
		6,               // Spreadi-ngFactor [6: 64, 7: 128, 8: 256, 9: 512, 10: 1024, 11: 2048, 12: 4096  chips]
		...
		250,              // TxPacketTimeout
		250,              // RxPacketTimeout
		9,              // PayloadLength (used for implicit header mode) - THIS IS OVERWRITTEN IN MAIN.C
};
...
void SX1276LoRaInit( void )
{
	...
	SX1276LoRaSetPayloadLength( LoRaSettings.PayloadLength ); // This is overwritten in main.c
	...
	else
	{
		...
		LoRaSettings.Power = 14; //This has been changed from 20 to 14 to meet EU laws
		SX1276LoRaSetRFPower( LoRaSettings.Power );
	}
...
void SX1276LoRaReset( void )
{
	...
	// Wait 1ms
	uint32_t startTick = linux_hal_get_ms();
	while( ( linux_hal_get_ms() - startTick ) < TICK_RATE_MS( 1 ) );

	SX1276SetReset( RADIO_RESET_OFF );

	// Wait 6ms
	startTick = linux_hal_get_ms();
	while( ( linux_hal_get_ms() - startTick ) < TICK_RATE_MS( 6 ) );
}
...
uint8_t SX1276LoRaGetModemStatus( void )
{
	SX1276Read( REG_LR_MODEMSTAT, &SX1276LR->RegModemStat );
	return SX1276LR->RegModemStat & ~RFLR_MODEMSTAT_MODEM_STATUS_MASK;
}

bool hederInfoValid = false;
bool rxOnGoing = false;
bool signalSynchronized = false;
bool signalDetected = false;
bool modemClear = false;
char *timeNowP;	
/*!
...
uint32_t SX1276LoRaProcess( void )
{
	...
	if(RFLR_STATE_RX_INIT || RFLR_STATE_RX_RUNNING || RFLR_STATE_RX_DONE)
	{
		// Get receiver events 
		getEpochTime(&timeNowP);

		if(!rxOnGoing && SX1276LoRaGetModemStatus() == 0x4){
			rxOnGoing = true;
			printf("\nrxOnGoing:%s;\n", &timeNowP);
		}
		else if(!signalDetected && SX1276LoRaGetModemStatus() == 0x5){
			signalDetected = true;
			printf("signalDetected:%s;\n", &timeNowP);
		}
		else if(!hederInfoValid && SX1276LoRaGetModemStatus() == 0xf){
			hederInfoValid = true;
			printf("headerInfoValid:%s;\n", &timeNowP);
		}	
		/*else if(!modemClear && SX1276LoRaGetModemStatus() == 0x10){
			modemClear = true;
			printf("ModemClear:%s;\n", &timeNowP);
		}*/
		/*else if(SX1276LoRaGetModemStatus() != 0x5 && SX1276LoRaGetModemStatus() != 0x4 && SX1276LoRaGetModemStatus() != 0xf && SX1276LoRaGetModemStatus() != 0x10){
			printf("Something else: 0x%x\n", SX1276LoRaGetModemStatus());
		}*/
	}
	...
	case RFLR_STATE_RX_INIT:
		...
		RxTimeoutTimer = linux_hal_get_ms();
	...
	case RFLR_STATE_RX_RUNNING:
		...
		if( LoRaSettings.RxSingleOn == true ) // Rx single mode
		{
			if( ( linux_hal_get_ms() - RxTimeoutTimer ) > PacketTimeout )
			{
	...
	case RFLR_STATE_RX_DONE:
		...
		hederInfoValid = false;
		rxOnGoing = false;
		signalSynchronized = false;
		...
		//printf("RSSI:%f\tSNR:%d\n", RxPacketRssiValue, RxPacketSnrEstimate);
